...
# Function to move files and report (only files directly in Downloads, not in subfolders)
move_files() {
    local pattern="$1"
    local destination="$2"
    local description="$3"
    
    # Only process files directly in Downloads folder (-maxdepth 1 -type f)
    files_moved=$(find . -maxdepth 1 -type f $pattern 2>/dev/null | wc -l | tr -d ' ')
    if [ "$files_moved" -gt 0 ]; then
        find . -maxdepth 1 -type f $pattern -exec mv {} "$destination"/ \; 2>/dev/null
        echo "Moved $files_moved $description to $destination/"
    fi
}
